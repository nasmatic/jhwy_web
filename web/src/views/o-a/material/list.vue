<template>
    <WaterMark>
        ...
        <Row class="margin-top-10">
            <Button icon="md-download" @click="downloadMaterialExcel">导出台帐</Button>
            <Button icon="md-download" @click="downloadPurchaseExcel">采购记录</Button>
            <Button icon="md-download" @click="downloadUsedExcel">领用记录</Button>
        </Row>
...
export default {
                ...
                },
                {
                    ...
                    render: (h, p) =>
                        h(
                            ...
                            {
                                on: {
                                    click: () => this.$router.push(`/o-a/hr/colleague/detail/${p.row.created_by}`)
                                }
                            },
                            p.row.real_name
                        )
    ...
    methods: {
        ...
        downloadMaterialExcel() {
            ...
            const ws = XLSX.utils.json_to_sheet(this.materialAllData, {
                header: ['id', 'name', 'total', 'category', 'storehouse', 'real_name', 'created_at']
            });
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
            // save
            const wbout = XLSX.write(wb, { type: 'binary', bookType: 'xlsx' });
            XLSX_SAVE.saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), '全部物料.xlsx');
        },
        downloadPurchaseExcel() {
            utils.request
                .post('/material/purchase_all', {
                    page_num: 1,
                    page_size: 10000
                })
                .then(res => {
                    this.purchaseAllList = res.data.list;
                })
                .catch(() => (this.fetching = false));
            this.purchaseAllList.forEach(item => {
                if (item['origin'] === 0) {
                    item['origin'] = '采购';
            ...
            const ws = XLSX.utils.json_to_sheet(this.purchaseAllData, {
                header: ['name', 'total', 'origin', 'finish', 'real_name', 'created_at']
            });
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
            // save
        ...
        downloadUsedExcel() {
            utils.request
                .post('/material/used_all', {
                    page_num: 1,
                    page_size: 10000
                })
                .then(res => {
                    this.usdAllList = res.data.list;
                })
                .catch(() => (this.fetching = false));
            this.usdAllList.forEach(item => {
                if (item['created_at']) {
                    item['created_at'] = moment(new Date(item['created_at'])).format('MM/DD/YYYY');
                }
                this.usedAllData.push({
                    id: item['id'],
                    name: item['name'],
                    total: item['total'],
                    reason: item['reason'],
            ...
            const ws = XLSX.utils.json_to_sheet(this.usedAllData, {
                header: ['id', 'name', 'total', 'reason', 'real_name', 'created_at']
            });
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
            // save
            const wbout = XLSX.write(wb, { type: 'binary', bookType: 'xlsx' });
            XLSX_SAVE.saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), '领用记录.xlsx');
        }
    },
    computed: {
        ...mapGetters({
