        ...
        <Card dis-hover :bordered="false" title="入职流程" class="mt-16">
            <Row class="detail-row" v-if="detail.createInfo">
                <Col :lg="8" :sm="12" :xs="24">
                    ...
                    <div class="detail-content">
                        <router-link :to="`/o-a/hr/colleague/detail/${detail.createInfo.id}`">
...
export default {
            ...
            columns: [
                {
                    ...
                    render: (h, p) =>
                        h(
                            'a',
                            { on: { click: () => this.$router.push(`/o-a/hr/detail/${p.row.operated_user_id}`) } },
                            p.row.real_name
                        )
                },
                {
    ...
    methods: {
        ...
        goUpdate() {
            this.$router.push(`/o-a/hr/update/${this.$route.params.id}`);
        },
        updateStatus() {
            Modal.confirm({
                title: '请确认',
